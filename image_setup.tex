...
It is possible that the testbed administrator requires to add other
scripts to configure the \ac{Raspi}s besides the ones introduced here.
This would be tedious to distribute to all \ac{Raspi}s at a later stage.
We ease this task by making the \ac{Raspi}s fetch all the configuration
scripts during start up from a \texttt{.zip} file before running the
configuration scripts. The below script attempts to download/update the
scripts:
