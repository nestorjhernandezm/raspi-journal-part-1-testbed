...
In our case, we have our configuration scripts stored in a \texttt{.zip}
...
configuration file \texttt{\${ROOTDIR}/home/pi/rasp\_config/nodes.csv}. A
sample of our file is shown as follows:
...
\texttt{\${ROOTDIR}/home/pi/rasp\_config/nodes.csv}, we use the
...
Besides the scripts introduced up to this point of our procedure, it is
possible that the testbed administrator requires to add other scripts to
configure its \ac{Raspi}s. We ease this task by making the \ac{Raspi}s
fetch all the configuration scripts during start up from a \texttt{.zip}
file before running the configuration scripts. The role of this script
is the same as theThe below script attempts to download/update the
scripts:
...
\ac{Raspi} \ac{MAC} address is found in \texttt{nodes.csv}. Thus,
