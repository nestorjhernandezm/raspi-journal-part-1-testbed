...
% In some cases, \texttt{chroot} may not work properly. tgus, in this case, an
% alternative can be \texttt{proot}. It may be required to install it
% before changing the root with \texttt{apt-get install proot}. Then, it
% should possible for the testbed administrator to run the following:
...
long-runnning processes. When writing the image either to a memory card
or storing it remotely \todot{Are we going to store the image
into a memory card or remotely in a URL? Since this is not completely clear
yet.}, all these change that has been made to the image so far will exists in
all \ac{Raspi}s.