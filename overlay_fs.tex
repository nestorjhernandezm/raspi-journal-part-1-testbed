...
erased after a hypothetic reboot. On the other hand, to enable the
possibility of persistent changes, we overlay the upper filesystem that is
mounted in the \ac{Raspi} \ac{RAM}, i.e. \texttt{/tmp} as read / writable
on top of the lower root filesystem. Thus, reading a file may return a
...
\ac{RAM}. Thus, all the files stored here will disappear when the system is
rebooted.
...
Again, if the \texttt{----recursive} option is not available, the
procedure in Section~\ref{sec:umount} of the Appendices could be followed.
